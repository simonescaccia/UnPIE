...
def get_unpie(phase, training_step):
    params_loader = ParamsLoader(training_step)
    pie_preprocessing = PIEPreprocessing(params_loader.get_pie_params())
    data_loaders = pie_preprocessing.get_data_loaders(phase)
    params = params_loader.get_params(data_loaders, phase)
    unpie = UnPIE(params)
    return unpie
...
if __name__ == '__main__':
    ...
    if train_test < 2:
        unpie = get_unpie('train', training_step)
        unpie.train()
    if train_test > 0:
        unpie = get_unpie('test', training_step)
        unpie.test()
